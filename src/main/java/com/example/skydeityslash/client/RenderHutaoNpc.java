...
/**
 * 胡桃人形 NPC 渲染器：与芙宁娜 NPC 完全相同的坐姿模型 + 下移 0.65 + 隐藏名字，
 * 仅贴图换为 hutao_npc.png。
 */
public class RenderHutaoNpc extends LivingEntityRenderer<HutaoNpcEntity, FurinaNpcModel<HutaoNpcEntity>> {
    private static final ResourceLocation TEX = new ResourceLocation("skydeityslash", "textures/entity/hutao_npc.png");

    public RenderHutaoNpc(EntityRendererProvider.Context ctx) {
        super(ctx, new FurinaNpcModel(ctx.bakeLayer(ModelLayers.PLAYER_SLIM)), 0.5F);
    }

    @Override
    public ResourceLocation getTextureLocation(HutaoNpcEntity e) { return TEX; }

    @Override
    protected void setupRotations(HutaoNpcEntity entity, PoseStack poseStack,
    ...
    @Override
    protected boolean shouldShowName(HutaoNpcEntity entity) { return false; }
}
