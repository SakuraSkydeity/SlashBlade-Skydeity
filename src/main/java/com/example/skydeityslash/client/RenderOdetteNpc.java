...
/**
 * 奥黛塔人形 NPC 渲染器：与芙宁娜 NPC 完全相同的坐姿模型 + 下移 0.65 + 隐藏名字，
 * 仅贴图换为 odette_npc.png。
 */
public class RenderOdetteNpc extends LivingEntityRenderer<OdetteNpcEntity, FurinaNpcModel<OdetteNpcEntity>> {
    private static final ResourceLocation TEX = new ResourceLocation("skydeityslash", "textures/entity/odette_npc.png");

    public RenderOdetteNpc(EntityRendererProvider.Context ctx) {
        super(ctx, new FurinaNpcModel(ctx.bakeLayer(ModelLayers.PLAYER_SLIM)), 0.5F);
    }

    @Override
    public ResourceLocation getTextureLocation(OdetteNpcEntity e) { return TEX; }

    @Override
    protected void setupRotations(OdetteNpcEntity entity, PoseStack poseStack,
    ...
    @Override
    protected boolean shouldShowName(OdetteNpcEntity entity) { return false; }
}
